...
# <font color="#E74C3C">电阻 (Resistance)</font>

-   电流密度的定义是单位面积上通过的电流量。如果 `A` 是承载恒定电流 `I` 的导线的横截面积，那么电流密度 `J` 的计算公式为：
    $$
    J = \frac{I}{A}
    $$

-   可以证明，电流密度 `J` 与电场强度 `E` 成正比：
    $$
    J = \sigma E
    $$
    其中，`σ` (sigma) 是材料的**电导率 (conductivity)**。

-   设 `l` 为导线的长度，`V` 为导线两端的**电势差 (potential difference)**（即电压）。该电势差在导体内部产生一个恒定的电场 `E`。电势差 `V` 与电场 `E` 的关系如下：
    $$
    V = El
    $$

-   将 `E = V/l` 和 `J = I/A` 代入 `J = σE`，我们可以得到：
    `I/A = σV/l`
    因此，
    `V = [l/(σA)]I`
...
# <font color="#E74C3C">电阻 (续)</font>

-   电阻与导线的**长度**成**正比**，与导线的**横截面积**和材料的**电导率**成**反比**。

-   材料的**电阻率 (resistivity)** `ρ` (rho) 是**电导率 (conductivity)** `σ` (sigma) 的倒数：
    $$
    \rho = \frac{1}{\sigma}
    $$
...
-   电阻与导线的**长度**和**电阻率**成**正比**，与导线的**横截面积**成**反比**。

-   用电阻 `R` 来表示，我们之前的方程 `V = (l/σA)I` 可以写为：
    $$
    V = RI
    $$
    这就是**欧姆定律 (Ohm's Law)**。
