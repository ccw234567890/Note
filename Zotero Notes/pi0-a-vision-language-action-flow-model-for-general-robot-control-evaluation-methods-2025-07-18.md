...
### 1. 开箱即用（Out of the Box with Language Commands）

**定义**：指**不经过任何微调**，直接将预训练后的 π₀ 模型应用于新的任务，仅通过**自然语言指令**（language commands）来指定任务目标。
...
**意义**：这测试的是模型的**零样本泛化能力**（zero-shot generalization）——即预训练阶段学到的知识能否直接迁移到未见过的任务上。这类似于 GPT 系列模型在未见过的 NLP 任务上进行 zero-shot 评估。

### 2. 微调（Fine-tuning to Downstream Tasks）

**定义**：在预训练模型的基础上，使用**下游任务**（downstream tasks）的少量数据对模型参数进行**有监督微调**，使其适应特定任务。
...
其中：
- $\theta_{\text{pre}}$：预训练权重（初始化）
- $\mathcal{D}_{\text{task}}$：下游任务的数据集
- $L_\tau(\theta)$：流匹配损失函数
- $\theta_{\text{ft}}$：微调后的权重
...
微调的任务包括：
- **折叠衣物**（laundry folding）
- **清理桌面**（clearing a table）
- **将碗碟放入微波炉**（putting dishes in a microwave）
- **将鸡蛋装入纸盒**（stacking eggs into a carton）
- **组装盒子**（assembling a box）
- **装袋杂货**（bagging groceries）

**意义**：微调测试的是模型的**少样本适应能力**（few-shot adaptation）——即预训练学到的通用机器人操控知识能否通过少量任务特定数据快速适配到新任务。这与 LLM 的 fine-tuning 范式完全一致。
...
SayCan 是 Google 提出的一个经典框架，其中 LLM 负责**语义规划**（决定"做什么"），而底层策略负责**物理可行性**（决定"能不能做"）。π₀ 的高层语义策略采用了类似的思想，但底层执行器不再是简单的技能库，而是 π₀ 这个通用的 VLA 模型。
...
一个时间上延展的任务可以建模为**马尔可夫决策过程（MDP）**中的**分层策略**：
...
这三种评估方式可以类比为**数字信号处理系统的三种测试模式**：

1. **开箱即用** = 滤波器出厂时的**频率响应测试**——不调整任何参数，直接测量其在不同输入下的性能。
2. **微调** = **自适应滤波器**——根据特定应用场景（如特定房间的声学环境）调整滤波器系数，以优化在该场景下的性能。
3. **高层语义策略** = **分层控制系统**——类似于一个自动增益控制（AGC）系统，其中高层控制器（AGC 算法）根据信号强度动态调整低层参数（放大器增益），而低层执行器（放大器）负责实际的信号放大。在 π₀ 中，HL-VLM 是"高层控制器"，π₀ 是"低层执行器"。

## My Notes

π₀ 的三种评估方式构成了一个完整的评估体系：
- **开箱即用**测试了模型的**泛化能力**——预训练是否学到了足够通用的操控知识。
- **微调**测试了模型的**适应能力**——能否快速适配到特定任务。
- **高层语义策略**测试了模型的**组合能力**——能否与高层规划器协同完成复杂的长时域任务。

这种分层评估策略与 LLM 的评估范式高度一致（zero-shot → few-shot → chain-of-thought/planning），体现了机器人基础模型正在向 LLM 的成熟评估体系靠拢。
