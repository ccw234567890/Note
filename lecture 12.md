...
# 笔记：MCU定时器 (Timers) 与中断实践

> [!abstract] 核心纲要
> 本笔记旨在深入剖析微控制器（MCU）中用于精确时间控制的核心组件——**定时器 (Timer)**。我们将重点关注：
> 1.  **定时器是什么**: 理解其作为“电子计数器”的基本工作原理和“翻转 (Roll Over)”特性。
> 2.  **预分频器 (Prescaler)**: 了解为何需要它来“减慢”时钟，以测量更长的时间间隔。
> 3.  **定时器的实际应用**: 通过Mbed平台的API和代码实例，学习如何使用定时器，并理解它如何与**中断 (Interrupts)** 协同工作，实现高效的多任务处理。
...
> [!info]
> **定义**: **定时器 (Timer)** 是微控制器内部的一个**硬件电子计数器**，能够非常精确地测量时间间隔或在预定时间后触发事件。

> [!help] **工作机制**
> - **核心**: 定时器的本质是一个**寄存器 (Register)**。
> - **计数**: 它每接收到一个来自内部时钟源的**时钟脉冲 (Clock Pulse)**，其寄存器内的数值就**加一**。
> - **举例**: 如果一个定时器的时钟频率是1 KHz（即每秒1000个脉冲），那么定时器的计数值每增加1，就代表过去了 **1毫秒 (ms)**。

> [!warning] **“翻转” (Roll Over) 特性**
> > - **定义**: 任何定时器的计数值都有一个上限。当计数值达到其最大值后，下一个时钟脉冲会使其**“翻转”回0**，并重新开始计数。
> > - **上限**: 由定时器的**位数**决定。
> >   - **8位定时器**: 最大值为 $2^8 - 1 = 255$。当计到255后，下一个值是0。
> >   - **16位定时器**: 最大值为 $2^{16} - 1 = 65535$。当计到65535后，下一个值是0。
> > - **应用**: 定时器的“翻转”本身可以作为一个事件，来触发一个**中断**，从而实现周期性的任务。

---

### Ⅱ. 控制“滴答”速度：时钟预分频器 (Clock Prescaler)

> [!question]
> MCU的主时钟频率通常非常高（如STM32的80 MHz），这意味着定时器计数的“滴答”声会快得惊人（每12.5纳秒一次）。我们如何用它来测量更长的、人类感官级别的延时（如秒或毫秒）？

> [!success] **解决方案：预分频器 (Prescaler)**
> - **定义**: **预分频器**是一个硬件**分频器**，它位于主时钟源和定时器/计数器之间。
> - **作用**: 它将输入的高频时钟信号，**除以**一个可设置的系数（预分频值），然后将**减慢后**的低频时钟信号输出给定时器。
> - **直观比喻**: 预分频器就像一个“变速齿轮箱”，将发动机（主时钟）的高转速，转换成车轮（定时器）需要的、更慢但更有用的转速。

> [!example] **计算实例**
> > - **主时钟**: 80 MHz
> > - **预分频值**: 80
> > - **定时器接收到的时钟频率**: `80 MHz / 80 = 1 MHz`
> > - **定时器“滴答”一次的时间**: `T = 1 / f = 1 / 1 MHz = 1 微秒 (μs)`
> >
> > 通过预分频，我们成功地将定时器的基本计时单位从纳秒级“放慢”到了微秒级，使其更适合测量毫秒或秒级的延时。

---

### Ⅲ. 实践应用：Mbed中的定时器与中断

> [!tip] **1. Mbed `Timer` API**
> > Mbed平台提供了简洁的 `Timer` 类来操作硬件定时器。
>
| 函数 | 用途 |
| :--- | :--- |
| **`start()`** | 启动定时器 |
| **`stop()`** | 停止定时器 |
| **`reset()`** | 将定时器计数值清零 |
| **`read()`** | 获取从启动开始经过的时间（单位：秒） |
| **`read_ms()`**| 获取经过的时间（单位：毫秒） |
| **`read_us()`**| 获取经过的时间（单位：微秒） |

> [!cite] **2. 定时器与中断的协同工作**
> > 下面的代码示例完美地展示了嵌入式系统如何**并行处理**一个**时间触发**的任务和一个**事件触发**的任务。
> > ```cpp
> > /* Simple Interrupt Example */
> > #include "mbed.h"
> > 
> > InterruptIn button(D5); // 定义一个中断输入引脚D5
> > DigitalOut led1(D3);
> > DigitalOut led2(D4);
> > 
> > // --- 中断服务程序 (ISR) ---
> > void ISR1() {
> >     // 当中断被触发时，这段代码会被立即执行
> >     ThisThread::sleep_for(1s); // 延时1秒以观察效果
> >     led1 = !led1;              // 翻转led1的状态
> > }
> > 
> > int main() {
> >     // 将 ISR1 函数附加到按钮引脚的“上升沿”事件上
> >     button.rise(&ISR1);
> > 
> >     // --- 主循环 (Main Loop) ---
> >     while(1) {
> >         // 这个循环会一直独立运行
> >         led2 = !led2; // 翻转led2的状态
> >         ThisThread::sleep_for(250ms); // 延时250毫秒
> >     }
> > }
> > ```
> > - **主循环 (后台任务)**: `main` 函数中的 `while(1)` 循环是一个**时间触发**的后台任务，它让 `led2` 以250ms的间隔稳定地闪烁。CPU大部分时间都在执行这个循环。
> > - **中断 (前台任务)**: 当用户**按下按钮**时，一个**事件触发**的中断会产生。CPU会**立即暂停**主循环，跳转去执行 `ISR1` 函数（翻转`led1`），执行完毕后，再**无缝地返回**到主循环中被打断的地方继续执行。
> >
> > **这个例子清晰地展示了中断如何让一个系统能够**在执行常规周期性任务的同时，**还能对不可预测的外部事件做出快速响应**。

![[屏幕截图 2025-08-12 165148.png]]

![[屏幕截图 2025-08-12 165158.png]]

![[屏幕截图 2025-08-12 165215.png]]
