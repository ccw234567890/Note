...
# 笔记：SPI协议实战 - Mbed编程与波形分析

> [!abstract] 核心纲要
> 本笔记旨在深入剖析**SPI (串行外设接口)** 的实际应用。我们将重点关注：
> 1.  **协议对比**: 通过一个清晰的对比表，全面了解**UART, I2C, SPI**三大协议的核心差异。
> 2.  **Mbed代码实战**: 详细解析一个**SPI主从通信**的完整Mbed C++代码示例。
> 3.  **波形绘制**: 通过一个具体的例子，学习如何根据给定的参数，绘制出SPI通信的**时序波形图**。

---

### Ⅰ. 三大串行协议对比：UART vs. I2C vs. SPI

> [!example]
> 这张总结表可以帮助我们快速理解在不同场景下应该选择哪种协议。

| 协议 (Protocol) | UART | I2C | SPI |
| :--- | :--- | :--- | :--- |
| **复杂度 (Complexity)** | 简单 (Simple) | 连接多设备简单 | 从设备增多时变复杂 |
| **速度 (Speed)** | **最慢 (Slowest)** | 比UART快 | **最快 (Fastest)** |
| **设备数量** | 2个 (点对点) | 最多127个，但会变复杂 | 很多，但会变复杂 |
| **引脚数量** | **1** (单向) 或 2 (双向) | **2** (SDA, SCL) | **4+** (MOSI, MISO, SCLK, CS) |
| **双工模式** | **全双工 (Full Duplex)** | 半双工 (Half Duplex) | **全双工 (Full Duplex)** |
| **主从关系** | 点对点 (Single to Single) | **多主多从** | 单主多从 |

---

### Ⅱ. SPI实战：Mbed平台主从通信代码解析

> [!info]
> 这个项目展示了两个Mbed开发板，一个作为**主设备(Master)**，一个作为**从设备(Slave)**，如何通过SPI进行双向数据交换。

> [!help] **项目目标**
> - **Master**: 读取自己连接的两个开关的状态，将信息发送给Slave。同时，接收来自Slave的开关状态，并用LED灯显示出来。
> - **Slave**: 接收来自Master的数据，并用LED灯显示。同时，读取自己连接的两个开关的状态，准备好在下一次通信时发送给Master。

> [!tip] **关键API函数**
> - **Master (`SPI`类)**:
>   - `SPI(mosi, miso, sclk)`: 创建SPI主设备实例。
>   - `format(bits, mode)`: 配置数据位长度和SPI模式。
>   - `frequency(hz)`: 设置时钟频率。
>   - `write(value)`: **核心函数**。发送一个值，并**返回**从设备同时发回的值，实现全双工。
> - **Slave (`SPISlave`类)**:
>   - `SPISlave(...)`: 创建SPI从设备实例。
>   - `receive()`: 检查是否收到了新数据。
>   - `read()`: 从接收缓冲区读取数据。
>   - `reply(value)`: **核心函数**。将一个值预加载到发送缓冲区，以便在**下一次**Master发起通信时发送出去。

> [!success] **代码逻辑解析 (`while(1)`循环)**
> > **Master端**:
> > 1. 读取本地两个开关 `switch_ip1` 和 `switch_ip2` 的状态，并将其组合成一个字节 `switch_word`。
> > 2. 将片选 `cs` 拉低，选中Slave。
> > 3. 调用 `recd_val = ser_port.write(switch_word)`。在这一步，Master将自己的开关状态 `switch_word` 发送出去，**同时**将Slave发回的数据存入 `recd_val`。
> > 4. 将片选 `cs` 拉高，结束本次通信。
> > 5. 根据接收到的 `recd_val` 来点亮红/绿LED灯。
>
> > **Slave端**:
> > 6. 读取本地两个开关的状态，并将其组合成一个字节 `switch_word`。
> > 7. 调用 `ser_port.reply(switch_word)`，将自己的开关状态**预加载**到发送缓冲区，**准备**下一次被Master读取。
> > 8. 通过 `if (ser_port.receive())` 检查Master是否发来了新数据。
> > 9. 如果有新数据，则调用 `recd_val = ser_port.read()` 读取数据。
> > 10. 根据接收到的 `recd_val` 来点亮自己的红/绿LED灯。
...
> - 时钟速度: `20MHz`
> - 发送顺序: **最高有效位 (MSB) 优先**
> - SPI模式: 时钟空闲为**低电平**，数据在时钟的**上升沿**同步。

> [!check] **解题步骤**
> 1. **参数解码**:
>    - **数据**: `0xD4` 的8位二进制是 `11010100`。MSB优先，所以发送顺序是 `1`, `1`, `0`, `1`, `0`, `1`, `0`, `0`。
>    - **时钟周期**: `T = 1 / f = 1 / (20 * 10^6 Hz) = 50ns`。
>    - **SPI模式**: “空闲为低”表示 **CPOL=0**。“在上升沿同步（采样）”表示 **CPHA=0**。因此，这是 **SPI Mode 0**。
> 2. **波形绘制**:
>    - **CS (SS)**: 在整个8位数据传输期间，保持**低电平**。
>    - **SCLK**: 从低电平（空闲）开始，产生8个完整的时钟脉冲，每个周期50ns。
>    - **MOSI**: 在**Mode 0**下，数据必须在时钟的**上升沿之前**就准备好。因此，在每个时钟周期的**下降沿**（或低电平期间），MOSI线需要变为下一个要发送的比特位的电平，并在整个上升沿期间保持该电平。
>
> **最终波形图**:
>
> 