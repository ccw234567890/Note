...
- **右侧弧形箭头上的 `x identity`**: 代表**快捷连接（Shortcut Connection）**或**恒等连接（Identity Connection）**。这里的 `identity` 意味着输入 `x` 在这条路径上**没有经过任何改变**，是原封不动的[[paper/大二上/AI模型练习/新生项目/1.深度残差学习在图像识别中的应用ResNet/恒等映射]]。这条连接是整个残差学习的精髓。
