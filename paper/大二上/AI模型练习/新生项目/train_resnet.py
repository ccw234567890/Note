...
"""Train ResNet on ImageNet for cat-dog classification."""
...
from src.models import CrossEntropyWithLabelSmooth, define_net_resnet, load_ckpt, get_networks, train
...
if __name__ == '__main__':
    ...
    predict_from_net(net, test_list, config, show_title="ResNet pre training")
    ...
    print("ResNet training total cost {:5.4f} s".format(time.time() - start))

    # 训练后测试推理
    predict_from_net(net, test_list, config, show_title="ResNet after training")

    # 导出模型文件
    export_mindir(net, "resnet18")
