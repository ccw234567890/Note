...
> [!quote] 核心论述
> **[[paper/大二上/AI模型练习/新生项目/3.《Attention is all you need》Transformer/Recurrent Neural Networks (RNN)]]**: 一类适用于处理序列数据的神经网络，通过循环结构处理时序信息。
...
- **多对多 (Many-to-Many, 异步)**:
    ...
    - **应用**: **[[paper/大二上/Week Ⅰ/3.《Attention is all you need》Transformer/Sequence Transduction|序列转导]]**，如**机器翻译**。例如，先完整输入英文句子 "Hello world"，再输出中文句子“你好 世界”。这通常由一个 **Encoder-Decoder** 架构实现。
...
- **问题**: **[[梯度消失]]/[[梯度爆炸]]**。当网络按时间展开后，它变成了一个非常深的网络。在反向传播时，梯度需要从最后一个[[paper/大二上/AI模型练习/新生项目/3.《Attention is all you need》Transformer/时间步]]一路传回第一个时间步。在这个漫长的过程中，梯度会因为连乘效应而变得极小（消失）或极大（爆炸）。
