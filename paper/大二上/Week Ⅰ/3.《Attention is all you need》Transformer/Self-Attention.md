...
2. **并行计算**: 对每个词的上下文表示（如 zit​）的计算，可以**同时进行**，因为它们之间没有像 [[paper/大二上/Week Ⅰ/3.《Attention is all you need》Transformer/Recurrent Neural Networks (RNN)|RNN]] 那样的顺序依赖。这极大地提升了计算效率。
