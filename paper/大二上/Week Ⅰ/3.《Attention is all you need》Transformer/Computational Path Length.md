...
- **为什么这是一个巨大的优势？**
    ...
    - **[[paper/大二上/Week Ⅰ/3.《Attention is all you need》Transformer/Recurrent Neural Networks (RNN)|RNN]]**: 要关联句子的第一个词和最后一个词，信息/梯度信号需要走过**整个句子的长度**。路径长度是 O(N)，其中 N 是序列长度。这就是 RNN 难以处理长距离依赖的根本原因（“传话游戏”中信息会失真）。
        
    - **[[paper/大二上/Week Ⅰ/3.《Attention is all you need》Transformer/ByteNet|基于卷积(CNN)的模型]]**: 通过堆叠卷积层，路径长度约为 O(N/k)；使用空洞卷积可以优化到 O(logk​N)。虽然比 RNN 好，但路径长度依然随序列长度增长。
