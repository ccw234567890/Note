...
- **核心机制**: 在训练 [[残差网络 (ResNet)|ResNet]] 时，每个[[残差块 (Residual Block)|残差块]]都有一个“生存概率” p。在每次前向传播时，这个块有 p 的概率被**激活**（执行 F(x)+x），有 1−p 的概率被**丢弃**（直接变成一个[[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/恒等映射]]，即输出就是输入）。
