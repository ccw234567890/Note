...
- **总结 (Summary):**
    ...
    - 在实际情况中，[[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/解析：ResNet为何能轻松学习恒等映射]]不太可能是最优解，但本文的重构方法可能有助于对问题进行“预处理”，使其更容易优化。
...
- **总结 (Summary):**
     ...
     如果[[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/最优函数]]相比一个[[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/全零映射]]更接近于一个[[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/恒等映射]]，那么优化器应该更容易在恒等映射的基础上寻找一个小的扰动（即残差），而不是从零开始学习一个全新的函数。
...
- **原文 (Original):**
    ...
    - We show by experiments ([[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/Fig. 7]]) that the learned residual functions in general have small responses, suggesting that identity mappings provide reasonable preconditioning.
        
- **总结 (Summary):**
    
    - 作者通过实验（图7）表明，网络学习到的残差函数通常[[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/响应值]]都很小，这从经验上支持了“恒等映射提供了一个合理的预处理”这一观点。
...
- **知识点 (Knowledge Points):**
    ...
    - [[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/Residual Block]]: 暗示了残差学习是以“块”（block）为单位在网络中应用的，而不是应用到每一层。 #Paper/ResNet/Architecture
...
- **原文 (Original):**
    ...
    - A building block is shown in [[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/Fig. 2]].
...
- **知识点 (Knowledge Points):**
    ...
    - [[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/Residual Block]]: 提供了残差块的通用数学表达式，其中y是输出，x是输入，F是要学习的残差函数。 #Paper/ResNet/Architecture
...
- **知识点 (Knowledge Points):**
    ...
    - [[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/Projection Shortcut]]: 提出了一种当维度变化时使用的快捷连接变体，它使用一个线性变换（通常是1x1卷积）来调整x的维度。 #Paper/ResNet/Architecture
...
- **原文 (Original):**
    ...
    - Experiments in this paper involve a function F that has two or three layers ([[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/Fig. 5]]), while more layers are possible.
...
- **知识点 (Knowledge Points):**
    ...
    - [[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/Fully Connected Layer]]: 全连接层。 #AI/DeepLearning/Layers
...
- **知识点 (Knowledge Points):**
    ...
    - [[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/Downsampling]]: 降低特征图空间分辨率的操作。 #AI/DeepLearning/Operations
...
- **知识点 (Knowledge Points):**
    ...
    - [[paper/大二上/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/FLOPs (Floating Point Operations)]]: 衡量模型计算复杂度的常用指标。 #AI/Terminology/Evaluation