...
- **核心原因**：这并非[[过拟合]]，而是因为**优化困难**。一个极深的网络很难被训练好，梯度在传播中可能会出现问题（如[[梯度消失]]），导致网络连一个简单的**[[paper/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/解析：ResNet为何能轻松学习恒等映射]] (Identity Mapping)** 都学不会。
...
- **“...优化器只需简单地将非线性层的权重驱动至零...”**
    ...
    - 对于[[paper/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/梯度下降]]等优化算法来说，要让一个函数的输出 $F(x)$ 趋近于 0，最“自然”、最“轻松”的方式就是**将其所有权重参数都推向 0**。
...
- [[paper/Week Ⅰ/1.深度残差学习在图像识别中的应用ResNet/梯度下降]]
- [[梯度消失]]
