...
> [!quote] 核心论述
> **[[paper/AI模型练习/新生项目/3.《Attention is all you need》Transformer/Encoder-Decoder Architecture]]**: [[paper/Week Ⅰ/3.《Attention is all you need》Transformer/Sequence Transduction|序列到序列]]任务的标准框架，编码器负责理解输入序列，解码器负责生成输出序列。

---

## 1. 核心思想：“先理解，再表达”

编码器-解码器（Encoder-Decoder）架构是解决[[paper/Week Ⅰ/3.《Attention is all you need》Transformer/Sequence Transduction|序列转导（Seq2Seq）]]问题的标准“蓝图”。它将复杂的序列转换任务优美地拆分成了两个阶段。
...
- **过程**:
    - 通常由一个[[paper/AI模型练习/新生项目/3.《Attention is all you need》Transformer/Recurrent Neural Networks (RNN)|RNN]]（或 LSTM, GRU）实现。
...
- **基于 Transformer 的实现 (现代标准)**:
    ...
    - **优势**: 这种“想一句，看一眼原文”的模式，使得模型可以轻松处理非常长的序列，并取得了革命性的成功，成为现代几乎所有 [[paper/Week Ⅰ/3.《Attention is all you need》Transformer/Sequence Transduction|序列转导]] 任务的基石。
